...
import "./App.css";
import api from "./api"; // reusable API
...
function App() {
  ...
  const runFlow = async () => {
    ...
    try {
      ...
      setPrompt(""); // Clear input after running
    } catch (err) {
      console.error(err);
      alert(
        err.response?.data?.error || "Error fetching AI response. Try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  const saveData = async () => {
    ...
    setSaving(true);
    try {
      await api.post("/save", {prompt: promptNode, response: responseNode});
